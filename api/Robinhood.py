...
class Robinhood:
    ...
    def place_order(self, instrument, quantity=1, bid_price = None, transaction=None):
        ...
        data = 'account=%s&instrument=%s&price=%f&quantity=%d&side=buy&symbol=%s&time_in_force=gfd&trigger=immediate&type=market' % \
               (urllib.quote(self.positions[0]['account']), urllib.unquote(instrument['url']), \
                float(bid_price), quantity, instrument['symbol'])
